...
class Visualizer:
    ...
    def build_panel(
        ...
        # Addition blobs — primary defect signal
        for bx, by, bw, bh in result.addition_bboxes:
            sx1, sy1 = int(bx * scale_x), int(by * scale_y)
            sx2, sy2 = int((bx + bw) * scale_x), int((by + bh) * scale_y)
            cv2.rectangle(live_annotated, (sx1, sy1), (sx2, sy2), _ORANGE, 2)
            cv2.putText(live_annotated, "ADD", (sx1, max(sy1 - 2, 8)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35, _ORANGE, 1, cv2.LINE_AA)
        # Background debris blobs (shown when DEBRIS_DETECTION_ENABLED = True)
